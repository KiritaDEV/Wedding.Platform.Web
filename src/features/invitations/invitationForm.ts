...
export function effectiveInvitationName(customName: string, guests: Array<Pick<GuestDraft, 'firstName' | 'lastName'>>): string {
  const custom = customName.trim().replace(/\s+/gu, ' ')
  if (custom) return custom
  const names = guests.map(guestFullName).filter(Boolean)
...
export function newGuestDraft(rowKey: string = createInvitationDraftId('guest')): GuestDraft {
  return { rowKey, firstName: '', lastName: null, relationship: 'guest_other', side: 'unspecified', weddingRoleIds: [], customWeddingRoleKeys: [] }
}
...
export function buildInvitationPayload(draft: InvitationFormDraft, draftRoles: DraftWeddingRole[]): InvitationMutationPayload {
